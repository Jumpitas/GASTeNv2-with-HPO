...
def get_fashion_mnist(root, train=True):
    return torchvision.datasets.FashionMNIST(root, download=True, train=train,
                                             transform=_norm((0.5,), (0.5,)))

def get_cifar10(root, train=True):
    return torchvision.datasets.CIFAR10(root, download=True, train=train,
                                        transform=_norm((0.485, 0.456, 0.406),
                                                        (0.229, 0.224, 0.225)))

def get_stl10(root, train=True):
    split = "train" if train else "test"
    return torchvision.datasets.STL10(root, download=True, split=split,
...
def get_chest_xray(root, train=True):
    split = "train" if train else "test"
    ds = hf_load_dataset("keremberke/chest-xray-classification", "full",
                         split=split, cache_dir=root)
    tfm = T.Compose([T.Lambda(lambda im: im.convert("RGB")),
                     T.Resize((128, 128)),
...
def get_imagenet(root, train=True, *, streaming=False, use_local_folders=False):
    ...
    if use_local_folders:
        ...
        ds = hf_load_dataset("imagefolder", data_dir=str(folder), split="train")
    else:
        ds = hf_load_dataset("imagenet-1k", split=split, cache_dir=root,
    ...
    class IM(Dataset):
        ...
        @property
        def targets(self):
            return torch.tensor([self.d[i]["label"] for i in range(len(self.d))])
