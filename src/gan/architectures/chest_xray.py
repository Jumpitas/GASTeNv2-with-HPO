...
class Generator(nn.Module):
    def __init__(self,
        ...
        in_ch = fmap*16
        blocks, to_rgbs = [], []
        for _ in range(log-2):                # 5 blocks → 128
...
# ────────── Builders ──────────
def build_cxr_g(z_dim: int = 128, base_ch: int = 64):
    return Generator(z_dim=z_dim, img_size=(1,128,128), fmap=base_ch)

def build_cxr_d(base_ch: int = 64):
